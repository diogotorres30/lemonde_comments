...
csv_cnt = 0
line_cnt = 0
csvFile = open(sys.argv[1] + ' from ' + sys.argv[2] + ' to ' + sys.argv[3] + ' part' + str(csv_cnt) + '.csv','w',newline='', encoding="utf-8")
...
def comment_extraction():
    ...
    for link in comment_links:
        ...
        for com in comment_cauldron.find_all(attrs={"itemprop": "commentText"}):
            ...
            line_cnt += 1
            csv_list[csv_cnt].writerow([comment_id, comment_text, user_name, comment_date, link])
            if(line_cnt > 1000000):
                csvFile.close()
                csv_cnt += 1
                csvFile = open(sys.argv[1] + ' from ' + sys.argv[2] + ' to ' + sys.argv[3] + ' part' + str(csv_cnt) + '.csv','w',newline='', encoding="utf-8")
        ...
        if(not comment_cauldron.find(attrs={"class": "next obf"})):
            continue
        while(1):
            page_content = retry(link.replace(".html","_" + str(i) + ".html"), "").decode("utf-8")
            comment_cauldron = BeautifulSoup(page_content, 'html.parser')
            for com in comment_cauldron.find_all(attrs={"itemprop": "commentText"}):
                comment_text = com.find(lambda tag: tag.name == 'p' and not tag.attrs).getText() + "\n"
                comment_id = com.attrs["data-reaction_id"]
                ...
                line_cnt += 1
                csv_list[csv_cnt].writerow([comment_id, comment_text, user_name, comment_date, link])
            csv_list[csv_cnt].writerow([comment_id, comment_text, user_name, comment_date, link.replace(".html","_" + str(i) + ".html")])
            if(line_cnt > 1000000):
                csvFile.close()
                csv_cnt += 1
                csvFile = open(sys.argv[1] + ' from ' + sys.argv[2] + ' to ' + sys.argv[3] + ' part' + str(csv_cnt) + '.csv','w',newline='', encoding="utf-8")
